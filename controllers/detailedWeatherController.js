...
const analyzeWeatherWarnings = (weatherData) => {
  ...
  if (weatherData.wind.speed > 20) {
    warnings.push(
      `Cảnh báo gió mạnh: ${weatherData.wind.speed}km/h. Hạn chế ra ngoài.`
    )
  }
  ...
  if (weatherData.uvIndex.value > 8) {
    warnings.push(
      `Chỉ số UV rất cao (${weatherData.uvIndex.value}). Tránh tiếp xúc trực tiếp với ánh nắng.`
    )
  }
  ...
  if (weatherData.rainfall.current > 10) {
    warnings.push(
      `Mưa to: ${weatherData.rainfall.current}mm. Có thể gây ngập lụt cục bộ.`
    )
  }
  ...
  if (weatherData.airQuality.index > 150) {
    warnings.push(
      `Chất lượng không khí kém (${weatherData.airQuality.index}). Hạn chế hoạt động ngoài trời.`
    )
  }
...
// Fetch detailed weather data from OpenWeatherMap API with support for coordinates or location name
const fetchDetailedWeatherData = async (params) => {
  try {
    // Get basic weather data
    let weatherUrl = `${process.env.WEATHER_BASE_URL}weather`

    // Handle different location parameter types
    if (params.lat && params.lon) {
      // Use coordinates if provided
      weatherUrl += `?lat=${params.lat}&lon=${params.lon}`
    } else {
    ...
    // Add API key
    weatherUrl += `&appid=${process.env.WEATHER_API_KEY_STUDENT_PACKAGE}`
    ...
    const condition =
      weatherConditionMap[weatherData.weather[0].main] ||
      weatherData.weather[0].description

    // Create location key - just use city name without country code
    const locationKey = weatherData.name
    ...
    const pressureRotation =
      pressure < 1000 ? '-45deg' : pressure > 1020 ? '45deg' : '0deg'
    ...
    if (weatherData.rain && weatherData.rain['1h']) {
      ...
      rainfallForecast = `Dự kiến còn mưa ${Math.round(
        rainfallCurrent * 0.7
      )} mm trong 24h tới.`
    ...
    const detailedWeather = {
      ...
      country:
        weatherData.sys.country === 'VN' ? 'Việt Nam' : weatherData.sys.country,
...
exports.updateDetailedWeatherData = async (req, res) => {
  try {
    ...
    const weatherData = await fetchDetailedWeatherData(locationParams)
    ...
    const weather = await DetailedWeather.findOneAndUpdate(query, weatherData, {
      new: true,
      upsert: true
    })
...
exports.getBadWeather = async (req, res) => {
  try {
    ...
    if (locationParams.lat && locationParams.lon) {
      ...
      weather = await DetailedWeather.findOne({
        'coordinates.lat': {
          $gte: locationParams.lat - 0.01,
          $lte: locationParams.lat + 0.01
        },
      ...
      weather = await DetailedWeather.findOne({ location: cityName })
    }
    ...
    if (
      ...
      weather = await this.updateDetailedWeatherData({ query: locationParams })
    }
