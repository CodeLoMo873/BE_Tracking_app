...
const initBadWeatherScheduler = () => {
  ...
  cron.schedule('*/15 * * * *', async () => {
    console.log('Running scheduled detailed weather update - ' + new Date().toLocaleString());
    try {
      await detailedWeatherController.updateDetailedWeatherData({ query: { city: 'Hanoi' } });
